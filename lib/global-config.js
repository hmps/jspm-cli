...
if ( !exports.config ) {
  exports.config = {};
  if (HOME)
    save(exports.config, config.current);
}
...
exports.save = save;
function save(configToWrite, saveTo) {
  saveTo = saveTo || getPlaceToSave();
  ...
  if ( saveTo === 'local' && !config.local.path ) {
    config.local.path = getDefaultLocalConfigPath();
    dprepend(configToWrite, defaultConfig);
  }

  try {
    fs.writeFileSync(config[saveTo].path, JSON.stringify(configToWrite, null, 2));
  }
  catch(e) {
...
exports.set = set;
function set(name, val, saveTo) {
    var nameParts = name.split('.');
    var configUpdate = config[saveTo].content;
    ...
    save(config[saveTo].content, saveTo);
}

exports.getPlaceToSave = getPlaceToSave;
function getPlaceToSave() {
