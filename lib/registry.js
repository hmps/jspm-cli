...
exports.configure = function(registry, saveTo) {
  var RegistryClass;
  var updatedConfig = getGlobalConfig();
  ...
  .then(function() {
    globalConfig.save(updatedConfig, saveTo === 'local');
  });
};

// jspm registry create mycompany jspm-github
// creates a custom registry based on the given handler
exports.create = function(name, handler, override, saveTo) {
  var updatedConfig = getGlobalConfig();
  ...
  if (!override && updatedConfig.registries[name]) {
    if (updatedConfig.registries[name].handler === handler)
      ...
      .then(function(configure) {
        if (configure)
          return Promise.resolve(exports.configure(name, saveTo))
          .then(function() {
      ...
      .then(function(override) {
        if (override)
          return Promise.resolve(exports.create(name, handler, true, saveTo));
        return false;
      });
  }
  ...
  return exports.configure(name, saveTo);
};


function getGlobalConfig(saveTo) {
    var updatedConfig;

    if ( saveTo === undefined ) {
      saveTo = globalConfig.getPlaceToSave();
    }
